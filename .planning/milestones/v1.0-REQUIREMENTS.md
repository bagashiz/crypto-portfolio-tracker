...
- [x] **LAYOUT-01**: User can run `layout-builder --build` to create the Dashboard and Transaction Log tabs with headers, formatting, frozen header rows, summary rows, and formulas, authenticated via the service account _(Validated live 2026-06-21)_
- [x] **LAYOUT-02**: User can run `layout-builder --update` to idempotently re-apply structure, formats, validations, and formulas without ever altering transaction data rows _(Validated live 2026-06-21 — round-trip confirmed data rows survive byte-for-byte; pre-clear idempotency fixed in quick task 260621-m70)_
