...
- **Holdings** — the core table (a formatted Table, so formulas use structured refs like `Holdings[Value]`). One row per asset. Live cells are computed by the Apps Script custom functions, routed by the **Network** column:
  ...
  - `Value = Qty × Price`; `Cost Basis`/`Real. PnL` via a **moving-average-cost state machine** (`assetPnlState`, `sheets/lib.ts`, shared with Summary — see below) that scans an asset's Transactions rows in chronological order, tracking (qty, avg cost/unit, cumulative realized PnL): a BUY updates the running average, a SELL locks in realized PnL against the *current* average and never revises it — so a closed sale's PnL can't mutate when a later, unrelated BUY reopens the position. `Cost Basis = qty × avg` (naturally $0 once a position is fully closed). `Val. %` = Value ÷ Σ Value (the asset's actual share, sits beside `Value`); `Tgt. %` is a hand-set target; `Tgt. Value = Tgt. % × Σ Value` (the target dollar position per asset — how much *should* sit in it); `Dev. % = Val − Tgt`; `Dev. Value = Value − Tgt. Value` (standard finance deviation convention: **+ve = overweight** (holding more than target), **−ve = underweight** (holding less) — NOT an action signal, just how far off target you are); `Unreal. PnL = Value − Cost Basis`. Column order interleaves each `%` with its `Value` pair, then groups Cost Basis with the PnLs: `Value, Val. %, Tgt. %, Tgt. Value, Dev. %, Dev. Value, Cost Basis, Unreal. PnL, Real. PnL`.
  - The **Ticker/Mint** column is the lookup identifier passed as the function argument, and it is *not* the display symbol: Hyperliquid rows use the **Hyperliquid spot ticker** (BTC→`UBTC`, HYPE→`HYPE`), Solana rows use the **SPL token mint address**.
...
- **Summary** — the dashboard (`summary.ts`): a USD→IDR rate (via `GOOGLEFINANCE("CURRENCY:USDIDR")`) and a **Risk Profile** at `D2:F2` (target-weighted risk score on a 1–10 scale — tiers map to 2/4/6/8/10 — plus a tier label whose chip is color-coded green→red by conditional formatting), headline KPIs in USD + IDR, allocation-by-Category (with $-to-rebalance), risk breakdown, and three charts. All pure `Holdings[…]`/`Transactions[…]` rollups — no Apps Script needed. Styling (banner, olive bands, zebra, borders, hidden gridlines, PnL + risk-tier conditional colors) is code-managed too.
  - The category table's **`Dev %` follows the same Actual − Target convention as Holdings** (+ve overweight, −ve underweight), but **`Rebalance $` is intentionally the opposite sign** (`= (Tgt% − Act%) × Total`, not a multiple of `Dev %`) — it's an action figure (+ve buy this much, −ve trim this much), not a deviation figure. Don't "simplify" it back to `Dev % × Total` — that would silently flip its meaning.
  - The **headline `Realized PnL` KPI is sourced from the FULL Transactions ledger**, not `SUM(Holdings[Real. PnL])`: `MAP(UNIQUE(Transactions[Asset]), …)` runs `assetPnlState` (`sheets/lib.ts`) over every distinct asset that ever appears in Transactions and sums each one's realized component. A Holdings-based total silently drops to $0 the instant a delisted asset's row is removed, even though its already-realized gains/losses still sit in the ledger. `Unrealized PnL` stays Holdings-based (a delisted position has none left to realize).
...
**One scheduled trigger** drives historical snapshots: `snapshotPortfolio()` computes portfolio totals and appends a row to the **History** tab. It fetches **Total Value fresh** (price/balance are custom functions, which go stale when the sheet isn't open — a trigger must not just read those cells) but reads **Cost Basis / Real. PnL** from the Holdings columns (the moving-average-cost formula, always fresh — no custom function/cache involved), resolved **by header name** (not fixed index — a Holdings column reorder previously mis-read `Val. %` as `Cost Basis`). The trigger is **not** created by pushing code — run `setupDailySnapshotTrigger()` **once** from the Apps Script editor to install it (daily ~00:00 Asia/Jakarta); re-running it replaces rather than duplicates. Triggers bind by **function name**, so pushing a `Code.gs` change alone is enough for the next scheduled fire to run the new logic — no reinstall needed unless the trigger itself is missing or its schedule changes. The **spreadsheet timezone is `Asia/Jakarta`** (set to match the script's) so the appended `new Date()` displays in WIB — if it drifts to another zone, the timestamps render shifted (e.g. `America/Los_Angeles` showed a 23:xx fire as 9:xx).

**A `Portfolio` custom menu** (`onOpen()`, a simple trigger — no auth needed just to build the menu) offers two items, both re-rendering the menu via `refreshPortfolioMenu()` when done so a label can flip immediately:
- **Refresh prices** (`refreshPortfolio`) — drops the `HL_PRICES`/`HL_BALANCES`/`JUP_BALANCES`/per-mint `JUP_PRICE_*` cache keys, then blanks/flushes/restores the Holdings `Qty.`/`Price` formulas (`forceHoldingsRecalc`) so the custom functions refetch immediately instead of waiting out the cache TTL.
- **Hide/Show amounts** (`togglePrivacyMode`) — masks dollar/IDR figures across **Holdings, Transactions, History, and Summary** with a solid black "redaction bar" (background + font set to the same color). Holdings' **Price column is deliberately excluded** — it's a per-unit market price (same for anyone holding that asset), not a position size, so masking it wouldn't hide anything sensitive; only Value onward (which reveals how much is actually held) is masked. Menu-only control, no visible checkbox: state lives in a bare, undecorated **`Z1`** cell on each tab (Summary's is the master, the other three are followers `syncPrivacyFollowers` mirrors on every toggle). This is a **pure visual overlay via conditional formatting** — no cell value, formula, or number format is ever touched, so nothing downstream (Val. %, Summary's rollups, the charts) can break. Two earlier approaches were tried and rejected: swapping the number format directly fails because Sheets Tables lock the format of a *typed* column (Holdings/Transactions' `CURRENCY` columns reject `setNumberFormat` via the API, even though the identical PnL green/red CF rules work fine on those same columns); overwriting cell content with literal `***` text works but cascades `#VALUE!` errors into every formula that reads a masked cell (Val. %, Summary's `SUM`/`SUMIF`s, the pie chart). **A CF `CUSTOM_FORMULA` rule cannot reference another sheet at all** (confirmed against the live API — even a same-sheet-qualified formula like `='Summary'!$Z$1` is rejected when the rule lives on a *different* tab), which is why each tab needs its own local `Z1` rather than all four pointing at one shared cell. The mask rule is added with `index: 0` (top priority) in each builder module so it overrides the PnL green/red rules when active. Summary's masked ranges include its category-table **Total row** (row 19) — easy to drop by off-by-one (`CAT_TOTAL` vs `CAT_TOTAL - 1`) since the Total row sits one past the last category row.

**The Sheets API silently CLAMPS a request's range to the sheet's current `gridProperties.rowCount` at apply time** — a "generously bounded" range like `endRowIndex: 1000` only actually sticks if the sheet's grid is already that tall. Transactions/Holdings are both kept at their exact data size (9 rows), not grown to 1000, so their privacy-mask CF range is silently trimmed to 9 regardless of what the builder code requests; a ledger row added past the current bottom won't be covered until the rule is re-applied. Don't "fix" this by growing the grid — it's a deliberate size, not an oversight.

**Transactions' `Amount` column is intentionally left UNTYPED in the Table** (`Price`/`Fees` are `columnType: "CURRENCY"`, `Amount` is not) — a *typed* column's number format is locked by the Sheets API (can't `setNumberFormat` a cell in it after the fact, even though CF rules work fine on it), so `Amount`'s USD format is applied separately via a plain `repeatCell`, matching how Summary formats its currency cells without any Table at all.
...
- **Sheet-managed** (the live balances/prices the formulas fetch, and the **Transactions ledger rows**) → runtime/user data. Add transactions in the sheet, not in code; the ledger in `transactions.ts` is a point-in-time `SEED` snapshot for reproducibility, not the source of truth. Applying `transactions --reset` **overwrites the live ledger with that placeholder SEED** — never run it without backing up the real rows first and restoring them after.

**A Table does NOT auto-expand to cover a row written via the API the way it does when a user types in the row right below it in the UI.** Writing values into rows past a Table's current bound (e.g. restoring backed-up ledger rows beyond the original `addTable` range) leaves those rows structurally outside the Table: no column formatting/type, and any structured self-ref formula in them (like `Amount`'s `=Transactions[Qty.]*Transactions[Price]`) returns `#VALUE!` — worse, since they're outside the Table, other tabs' structured refs into it (Holdings' Cost Basis `SUMIFS(Transactions[Amount], …)`) silently skip them too. Fix by explicitly widening the Table's range first with an `updateTable` request (`fields: "range"`), then writing the values.

**Layout:** `sheets/apply.ts` resolves tab titles → sheetIds and runs each module (`bun run sheet:build [tab] [--reset] [--dry-run]`); per-tab modules are `holdings.ts`, `transactions.ts`, `summary.ts`, `history.ts`; `sheets/lib.ts` holds helpers (`valuesAt`, `writeValues`, `oneOfList`, `TABLE_BANDING`, `teardownRequests`, `gws`, `resolveSheetMeta`, `assetPnlState`). A module's `build()` returns `{ structure, values }`.
