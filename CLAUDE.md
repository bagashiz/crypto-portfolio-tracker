...
- **Holdings** — the core table (a formatted Table, so formulas use structured refs like `Holdings[Value]`). One row per asset. Live cells are computed by the Apps Script custom functions, routed by the **Network** column:
  ...
  - `Value = Qty × Price`; `Cost Basis` via `SUMIFS` over Transactions (Buys + fees − Sells); `Val. %` = Value ÷ Σ Value (the asset's actual share, sits beside `Value`); `Tgt. %` is a hand-set target; `Tgt. Value = Tgt. % × Σ Value` (the target dollar position per asset — how much *should* sit in it); `Dev. % = Val − Tgt`; `Dev. Value = Value − Tgt. Value` (standard finance deviation convention: **+ve = overweight** (holding more than target), **−ve = underweight** (holding less) — NOT an action signal, just how far off target you are); `Unreal. PnL = Value − Cost Basis`; `Real. PnL` = weighted-average realized P/L (sell proceeds − fees − avg buy cost × qty sold). Column order interleaves each `%` with its `Value` pair, then groups Cost Basis with the PnLs: `Value, Val. %, Tgt. %, Tgt. Value, Dev. %, Dev. Value, Cost Basis, Unreal. PnL, Real. PnL`.
