...
- **Hide/Show amounts** (`togglePrivacyMode`) — masks every dollar/IDR figure across **Holdings, Transactions, History, and Summary** with a solid black "redaction bar" (background + font set to the same color), driven by a checkbox at **Summary!H1**. This is a **pure visual overlay via conditional formatting** — no cell value, formula, or number format is ever touched, so nothing downstream (Val. %, Summary's rollups, the charts) can break. Two earlier approaches were tried and rejected: swapping the number format directly fails because Sheets Tables lock the format of a *typed* column (Holdings/Transactions' `CURRENCY` columns reject `setNumberFormat` via the API, even though the identical PnL green/red CF rules work fine on those same columns); overwriting cell content with literal `***` text works but cascades `#VALUE!` errors into every formula that reads a masked cell (Val. %, Summary's `SUM`/`SUMIF`s, the pie chart). **A CF `CUSTOM_FORMULA` rule cannot reference another sheet at all** (confirmed against the live API — even a same-sheet-qualified formula like `='Summary'!$H$1` is rejected when the rule lives on a *different* tab), so each masked tab carries its own local boolean follower cell at **`Z1`** (off past that tab's real content/Table/charts), and `togglePrivacyMode`/an `onEdit(e)` simple trigger keep all four cells mirrored to Summary!H1 (menu click and a direct click on the H1 checkbox both work). The mask rule is added with `index: 0` (top priority) in each builder module so it overrides the PnL green/red rules when active.
