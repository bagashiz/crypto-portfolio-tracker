 ...
 * no top-level `function refreshAll()` for the picker to discover; with the bundle
 ...
 * new entry point later is a one-line change in BOTH this namespace object and the
 * footer script's name array.
 *
 * Importing `Config` here ensures the inlined asset registry is included in the
 * bundle even though nothing references it directly.
 */
...
// Editor-callable entry points: the refresh + trigger globals.
...
(globalThis as any).__ENTRY__ = { refreshAll, installTrigger, removeTrigger };
...
// dependency, and makes the registry available to the refresh code.
...
// functions only. refreshAll() calls them from inside the bundle.
